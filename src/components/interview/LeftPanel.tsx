  ...
  return (
    <div
      className={`flex flex-col h-[90vh] overflow-y-auto ${
        sideBar ? "w-[30vw]" : "w-[5vw] bg-[#111111]"
      } items-center pt-10 `}
    >
      <div className="flex w-full justify-between items-center">
        {sideBar && <h2 className="text-lg font-bold">Chat Panel</h2>}
      ...
      {sideBar && (
        <>
          <div className="h-[30%]  text-white p-4 flex flex-col gap-3 overflow-y-auto w-full">
          ...
          <div className="flex flex-col gap-3 h-[70%]">
            <div className="flex flex-col gap-5  h-[90%] overflow-y-auto ">
              {[...messages].reverse().map((m, i) => (
                <span
                  key={i}
                  className=" font-medium text-[#777777] text-[15px]  bg-[#202020] rounded-md p-2 flex gap-2  font-mono"
                >
                  <p className="font-bold text-[12px] text-[#383838ee]">
                    {i + 1}
                  </p>
                  {m}
                </span>
            ...
            <form
              className="h-[50px] w-full flex gap-2 justify-center items-center "
              onSubmit={submitHandler}
            >
              <input
                type="text"
                className="w-[80%] bg-transparent border-2  outline-none rounded-md pl-5  border-[#474747] h-full  cursor-pointer"
                value={message}
